...
DEFAULT_CONFIG = {
    ...
    "appearance": {"favicon_url": "./favicon.png"},  # URL to your favicon
    "analytics": {"google_analytics_id": ""},  # Your Google Analytics ID (leave empty if not using)
}
...
def locate_about_function(file_path):
    return find_function_name_by_usage(
...
def update_app_name():
    ...
    return update_file_with_patterns("main.js", main_patterns) and update_file_with_patterns(
...
<script>
  ...
  gtag('config', '{GOOGLE_ANALYTICS_ID}');
        ...
        new_content = (
            re.sub(ga_script_pattern, new_ga_script, content)
            if re.search(ga_script_pattern, content)
            else re.sub(r"<head>", f"<head>\n{new_ga_script}", content)
        )
    else:
...
def update_favicon():
    ...
    for match in matches:
        ...
        if rel_match:
            ...
            new_link = f'<link rel="{rel}" {sizes_attr}href="{NEW_FAVICON_URL}">'
...
def update_about_text(config):
    about_text = config["project"].get(
        "about_text",
        f'<p class="mb-3">A clone of <a href="https://www.heardle.app" title="Heardle">Heardle</a> for {ARTIST_NAME} songs.</p>',
    )
